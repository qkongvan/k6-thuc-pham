...
class Manage_categoriesController extends HController
{
	...
	public function actionMoveUp($id)
	{
		...
		$this->redirect($this->createAbsoluteUrl('/products/manage_categories/admin', array('moving' => $id)));
	}
	...
	private function __getDataTreeChilds($childrens, $moving = null)
	{
		...
		foreach ($childrens as $index => $child) {
			...
			$linkDown = $index == $length - 1 ? '<span style="display:inline-block;width:16px"></span>' : '<a title="' . ProductsModule::t('Move Down') . '" href="' . $this->createAbsoluteUrl('/products/manage_categories/movedown', array('id' => $strChildId)) .
			...
			$linkDelete = '<a title="' . ProductsModule::t('Delete') . '" onClick="if (!confirm(\'' . ProductsModule::t('Are you sure you want to delete?') . '\')) return false;" href="' . $this->createAbsoluteUrl('/products/manage_categories/delete', array('id' => $strChildId)) .
