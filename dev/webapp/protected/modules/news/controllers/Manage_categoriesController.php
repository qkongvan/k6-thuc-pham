...
class Manage_categoriesController extends HController
{
	...
	public function actionCreate()
	{
		...
		$treeRoots = NewsCategory::model()->roots()->findAll();
		if (empty($treeRoots)) {
		...
		if (isset($_POST['NewsCategory'])) {
			...
			if ($model->parent_id == 'root') {
				...
				if ($model->validate()) {
					$root = NewsCategory::model()->findByAttributes(array('id' => $model->parent_id));
					$model->appendTo($root);
					$this->redirect(array('admin'));
				}
			}
	...
	public function actionMoveUp($id)
	{
		...
		$this->redirect($this->createAbsoluteUrl('/news/manage_categories/admin', array('moving' => $id)));
	}
	...
	private function __getDataTreeChilds($childrens, $moving = null)
	{
		...
		foreach ($childrens as $index => $child) {
			...
			$linkUp = $index == 0 ? '' : '<a title="' . NewsModule::t('Move Up') . '" href="' . $this->createAbsoluteUrl('/news/manage_categories/moveup', array('id' => $strChildId)) . '"><img src="' . $this->__baseScriptUrl . '/uparrow.png" /></a>';
			$linkDown = $index == $length - 1 ? '<span style="display:inline-block;width:16px"></span>' : '<a title="' . NewsModule::t('Move Down') . '" href="' . $this->createAbsoluteUrl('/news/manage_categories/movedown', array('id' => $strChildId)) .
				'"><img src="' . $this->__baseScriptUrl . '/downarrow.png" /></a>';
			$linkUpdate = '<a title="' . NewsModule::t('Update') . '" href="' . $this->createAbsoluteUrl('/news/manage_categories/update', array('id' => $strChildId)) . '"><img src="' . $this->__baseScriptUrl . '/update.png" /></a>';
			$linkDelete = '<a title="' . NewsModule::t('Delete') . '" onClick="if (!confirm(\'' . NewsModule::t('Are you sure you want to delete?') . '\')) return false;" href="' . $this->createAbsoluteUrl('/news/manage_categories/delete', array('id' => $strChildId)) .
