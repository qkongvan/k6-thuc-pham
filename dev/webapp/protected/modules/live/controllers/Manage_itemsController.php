...
class Manage_itemsController extends HController
{
	...
	public function actionCreate($category = '')
	{
		...
		if (isset($_POST['LiveItem'])) {
			...
			if (isset($_POST['LiveItem']['categories']))
				$model->categories = implode(',', $_POST['LiveItem']['categories']);
			...
			if (isset($fileUpload) && $model->validate()) {
				...
				$thumb->create($uploadPath . '/' . $filename)
					  ->adaptiveResize(Yii::app()->getModule('live')->widthThumb, Yii::app()->getModule('live')->heightThumb)
		...
		$categories = LiveCategory::model()->roots()->findAll();
	...
	{
		$model = LiveItem::model()->findByPk($id);
		if ($model === null)
